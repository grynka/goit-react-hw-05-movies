  ...
  return (
    <main>
        ...
        <List>
          {movies.map((movie) => (
            <Item key={movie.id} >
            <Link to={`/movies/${movie.id}`}><Image src={`https://image.tmdb.org/t/p/w780${movie.backdrop_path}`} alt={movie.title} width="300"></Image>{movie.title} ({movie.release_date.slice(0, 4)})</Link>
            </Item>
          ))}
        </List>
    </main>
